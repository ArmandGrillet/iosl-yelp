...
function getNumberOfFeaturesforRadius(type, business, radius, list) {
    ...
    if (elementsInRadius > 0) {
        console.log(elementsInRadius + " elements of type " + type + " in a radius of 100m.");
    }
...
getBusinesses('Edinburgh', 'Bars', function(businesses) {
    ...
    for (var i = 0; i < businesses.length; i++) {
        ...
        for (var j = 2; j < fields.length; j++) {
            row[fields[j]] = getNumberOfFeaturesforRadius(fields[j], businesses[i], 100, source[fields[j]]);
        }
        rows.push(row);
    }
