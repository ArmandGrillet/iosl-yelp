...
function get_hotspots(longitude, latitude) {
  ...
  var hotspots = [];
  var temp_hotspots = [];
  var no_of_shops = latitude.length;
  ...
  for (var i = 0; i < no_of_shops; i++) { // Loop to iterate through the list of latitudes and longitudes and calculate distance between them
    hotspot_count = 0;
    for (var j = 0; j < no_of_shops; j++) {
      ...
      if (relative_distance < 1 && relative_distance > 0) { // We have taken distance 3KM if the shops are neighbouring
        hotspot_count = hotspot_count + 1;
        temp_hotspots.push({ // Taking a temporary list to save the list of shops which are close to the base shop but not yet proved to be a hotspot
          'latitude': latitude[j],
    ...
    if (hotspot_count > 6) {
      for(
      var temp of temp_hotspots){ // Taking the coordinates from the temp list after they fulfil the condition of hotspot
      hotspots.push({
        'latitude': temp.latitude,
        'longitude': temp.longitude
      });}
      hotspots.push({ // capturing the coordinates of base shop from where we were calculating distances from other shops
        'latitude': latitude[i],
